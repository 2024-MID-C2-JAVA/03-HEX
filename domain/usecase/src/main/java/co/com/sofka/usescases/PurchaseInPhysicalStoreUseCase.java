...
public class PurchaseInPhysicalStoreUseCase implements UseCase<AddTransactionRequest, TransactionDoneResponse> {
    private final TransactionManager transactionManager;

    public PurchaseInPhysicalStoreUseCase(TransactionManager transactionManager) {
        this.transactionManager = transactionManager;
    }

    @Override
    public TransactionDoneResponse execute(AddTransactionRequest request) {
        return transactionManager.processTransaction(request.amount(), TransactionType.PHYSICAL_PURCHASE);
    }
}
